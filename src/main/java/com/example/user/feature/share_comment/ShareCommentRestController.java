...
public class ShareCommentRestController {
    ...
    public ResponseEntity<CustomPage<ShareCommentResponse>> getAllShareComments(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            HttpServletRequest request) {
        String baseUrl = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + "/api/v1/share-comments";
        CustomPage<ShareCommentResponse> shareCommentResponseCustomPage = shareCommentService.getAllShareComments(page, size, baseUrl);
